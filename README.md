...
│   ├── team.js          # 비공개 방 초대·멤버 관리
│   └── contact.js       # 문의 폼 접수
├── lib/
│   └── canvasShared.js  # server.js·api/action.js 공용 검증 상수·지우개 로직
...
- 기본 도형 도구 (사각형, 원, 삼각형, 화살표 — 노트 가장자리에 바인딩 가능)
- 선택 툴 (범위 드래그 → 다중 항목 일괄 이동)
- AI 이미지 변환 (이미지 더블클릭 → 프롬프트 입력 → HuggingFace instruct-pix2pix로 스타일 변환, `HF_TOKEN` 필요)
- Pusher 실시간 협업 (커서·획·포스트잇·도형 동기화)
- Google 로그인 / 로그아웃 (매번 계정 선택 강제)
- 프로젝트 사이드바 (전환·추가·삭제·휴지통·데이터 보존)
- 비공개 방 / 팀 초대 (초대 링크 발급 시 방이 비공개로 전환, 멤버만 접근)
...
- AI 이미지 생성 (빈 캔버스에서 프롬프트로 새 이미지 생성)
- AI 아이디어 제안
